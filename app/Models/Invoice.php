...
class Invoice extends Model
{
    ...
    {
        return $q->where('status', self::STATUS_UNPAID)->where('due_date', '<', Carbon::now());
    }
    ...
    {
        return $q->whereYear('issue_date', Carbon::now()->year)->whereMonth('issue_date', Carbon::now()->month);
    }
    public function scopeBetweenDates($q, $start, $end)
    {
        return $q->whereBetween('issue_date', [$start, $end]);
    }
    public function scopeForCustomer($q, $id)
    {
        return $q->where('customer_id', $id);
    }
    public function scopeForReservation($q, $id)
    {
        return $q->where('reservation_id', $id);
    }
    ...
    {
        return $this->status === self::STATUS_UNPAID && $this->due_date < Carbon::now();
    }
    ...
    {
        return !$this->is_overdue ? 0 : Carbon::now()->diffInDays($this->due_date);
    }
    public function getDaysUntilDueAttribute()
    {
        return ($this->is_paid || $this->is_overdue) ? 0 : $this->due_date->diffInDays(Carbon::now());
    }
    public function getCustomerNameAttribute()
    {
        return !$this->customer ? 'Client inconnu' : trim($this->customer->name . ' ' . ($this->customer->last_name ?? ''));
    }
    ...
    {
        return in_array($this->status, [self::STATUS_UNPAID, self::STATUS_OVERDUE]);
    }
    ...
    {
        if ($this->is_overdue && $this->status === self::STATUS_UNPAID) $this->update(['status' => self::STATUS_OVERDUE]);
    }
    ...
    public static function existsForReservation($id)
    {
        return self::where('reservation_id', $id)->exists();
    }
    ...
    protected static function boot()
    {
        ...
        static::retrieved(function ($inv) {
            $inv->updateOverdueStatus();
        });
    }
}
