...
class AdminApi {
  ...
  async createBalanceInvoice(depositInvoiceId) {
    try {
      const response = await httpClient.post(`/admin/invoices/${depositInvoiceId}/create-balance`)
      return response.data
    } catch (error) {
      console.error('Erreur création facture de solde:', error)
      throw this.handleError(error)
    }
  }
